...
| `schedule.py` | `POST /schedules`<br>`POST /schedules/normalize-preview`<br>`POST /schedules/{id}/routes`<br>`GET /schedules/{id}`<br>`GET /confirmed-schedules`<br>`GET /draft-schedules`<br>`PATCH /schedules/{id}/title`<br>`POST /schedules/bulk-delete`<br>`DELETE /schedules/{id}`<br>`POST`/`DELETE /schedules/{id}/required-places`<br>`GET /schedules/{id}/place-search`<br>`POST /schedules/{id}/required-places/custom`<br>`POST /schedules/{id}/candidates/{cid}/preview`<br>`POST /schedules/{id}/candidates/{cid}/preview/{pid}/save`<br>`POST /schedules/{id}/candidates/{cid}/removal/preview`<br>`POST /schedules/{id}/candidates/{cid}/removal/save`<br>`POST /schedules/{id}/candidates/{cid}/reorder/preview`<br>`POST /schedules/{id}/candidates/{cid}/reorder/save`<br>`POST /schedules/{id}/candidates/{cid}/activities/time/preview`<br>`POST /schedules/{id}/candidates/{cid}/activities/time/save`<br>`POST /schedules/{id}/candidates/{cid}/activities/{order}/unlock`<br>`POST /schedules/{id}/regenerate`<br>`POST /schedules/{id}/confirm` | Step1→2→3 실행해 경로 없는 후보 생성, 검색 풀과 정규화 조건을 저장<br>Step1만 실행해 좋아요·싫어요 정규화 결과·직접·의미 충돌·상한 초과 태그를 미리 보여줌. 확정 태그에 직접 충돌이 남으면 서버가 422로 거절<br>사용자가 고른 후보 1개에 Step4 실행해 이동 옵션 저장<br>세션 조회<br>확정 일정 목록(검색·필터링)<br>초안(미확정) 일정 목록<br>일정 이름 변경<br>여러 일정 한 번에 삭제<br>일정 하나 삭제<br>후보 풀의 장소를 `ScheduleRequiredPlace` 제약으로 추가·해제하고, 새 검색 없이 그 장소를 모두 포함하는 후보로 재생성(성공할 때만 기존 후보 교체)<br>표준 카테고리·태그 검색과 무관하게 이름으로 네이버 지역검색 직접 호출<br>검색 결과를 `is_custom=True` 필수 장소로 저장(최대 3개), 재생성 시 `place_candidates`에 원본 좌표로 직접 주입<br>후보의 장소 하나를 대체 장소로 바꾸는 미리보기<br>대체 미리보기 저장(교통편 변경 포함)<br>후보에서 장소 하나를 제외하는 미리보기<br>제외 미리보기 저장<br>후보 활동 순서를 드래그로 바꾸는 미리보기<br>순서 변경 저장<br>활동 시간 수동 수정 미리보기 — `travel_estimate.apply_manual_time()`으로 겹치는 안 잠긴 이웃을 밀고 잠긴 이웃과 겹치면 409<br>활동 시간 수동 수정 저장(`time_locked=True`로 고정)<br>잠긴 시간 해제(시간 값 자체는 안 바꿈)<br>재생성(필수 장소 반영)<br>후보 확정과 공유 링크 발급 | `pipeline/orchestrate.py`, `pipeline/normalize_step1.py`, `pipeline/enrich_step4.py`, `pipeline/travel_estimate.py`, `services/naver_local_search.py`, `services/naver_map_url.py`, `models/schedule.py` |
...
| `naver_map_url.py` | `build_naver_map_url(place)` — place_candidates 항목(title/address/roadAddress)으로 네이버 지도 검색 URL 조립(좌표 변환 없이 이름+주소 텍스트 검색). 층/호수·건물명이 붙은 주소는 도로명+건물번호만 남기도록 정리해야 검색이 실제로 맞는 장소를 찾음(실측 확인, 2026-08-09~11) |
