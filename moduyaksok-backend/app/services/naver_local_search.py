...
# 2026-08-15, "공원" 카테고리 추가(한강공원 등도 일정 후보로 탐색되게 해달라는
#             요청). 실측 결과 이 쿼리는 다른 카테고리보다 노이즈가 있다 —
#             "잠실 공원"은 헤어샵·주점까지 섞여 나오고 "홍대 공원"은 실제
#             공원 위주로 잘 나오는 등 지역에 따라 편차가 큼(순수 "시민공원"으로
#             바꿔봐도 특정 지역에서 0건이 나와 더 안 좋음). 그래도 다른
#             카테고리 검색과 마찬가지로 이후 Step2 LLM이 title/category를 보고
#             거르므로 후보 풀에 노이즈가 조금 섞이는 것 자체는 치명적이지
#             않다고 판단해 그대로 채택.
# ------------------------------------------------------------------
import asyncio
import hashlib
