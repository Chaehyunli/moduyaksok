# ------------------------------------------------------------------
# 작성자      : 임채현
# 작성목적    : Step 4 — 검증·병합 (MoA Aggregator). 단일 LLM 호출로 3개 후보를
...
def _similarity_score(a: EnrichedCandidate, b: EnrichedCandidate) -> float:
...
async def synthesize_and_validate(
    ...
    """Step 2~3에서 나온 3개 초안(장소·순서·시간·이동경로는 이미 확정됨)을 한 번의
    LLM 호출에 모두 전달해 판단만 한다 — 구조를 재배치하지 않는다:
