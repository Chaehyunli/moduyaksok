...
# 2026-08-14, 사용자 관측: title이 "보드게임 → 돼지고기구이 → 카페 → 체험 → 분식
#             코스"처럼 카테고리를 화살표로 기계적으로 나열하는 경우가 있었음 —
#             기존 지시("장소 목록을 잘 나타내면서...")가 구체적인 스타일을 안
...
카테고리를 "보드게임 → 돼지고기구이 → 카페 → 체험 → 분식 코스"처럼 화살표로 기계적으로 \
나열하지 마라 — 사람이 실제로 말하듯 자연스러운 짧은 문구로 써라(예: "홍대 보드게임 \
데이트 코스", "든든하게 먹고 노는 하루", "카페 투어와 방탈출"). 장소 개수만큼 카테고리를 \
전부 욱여넣을 필요 없다, 가장 특징적인 요소 한두 개만 담아도 된다.
