...
2026-08-17부터 클라이언트 패스프레이즈 암호화로 전환(`docs/superpowers/specs/
2026-08-17-byok-client-side-encryption-design.md`) — 서버는 `encrypted_key`를
스스로 복호화할 마스터키를 갖지 않는다.

| 컬럼 | 타입 | 제약 | 설명 |
|---|---|---|---|
| id | UUID | PK | |
| user_id | UUID | FK → user.id, UNIQUE, NOT NULL | 1인당 1개 키 |
| provider | VARCHAR | NOT NULL | `anthropic` \| `openai` \| `upstage` \| `google` — 사용자가 등록 화면에서 선택 |
| encrypted_key | BYTEA | NOT NULL | 클라이언트가 패스프레이즈 유도 키로 AES-GCM 암호화한 값. 평문 저장 금지, 서버는 해석 안 함 |
| salt | BYTEA | NOT NULL | PBKDF2 salt — 비밀 아님(복호화 재료) |
| iv | BYTEA | NOT NULL | AES-GCM iv — 비밀 아님(복호화 재료) |
| kdf_iterations | INTEGER | NOT NULL | 등록 당시 PBKDF2 반복 횟수(기본 600,000) — 나중에 값을 올려도 기존 행은 등록 당시 값으로 복호화 |
| masked_key | VARCHAR | NOT NULL | 클라이언트가 계산해 같이 보낸 표시용 마스킹 값(예: `sk-ant-••••••••uvwx`) |
